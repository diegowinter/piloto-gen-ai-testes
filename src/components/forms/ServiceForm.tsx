...
function ServiceForm(props: ServiceFormProps) {
  ...
  return (
    <div className="flex flex-col h-full w-full" >
      ...
      <div className="bg-slate-900 w-full p-3 my-6 rounded-xl">
        {!isHandbookGenerated ? (
          <div className="">
            <span className="text-xs opacity-80">Gravação da consulta</span>
            {filesContent[0] && !isSubmitting && (
              <div className="flex flex-col items-center justify-center h-full my-6">
                ...
                <span className="bg-slate-950 px-2 py-1 rounded-lg">{filesContent[0].name}</span>
              </div>
            )}
            {loading && (
              <div>Carregando...</div>
            )}
            {!filesContent[0] && !loading && !isSubmitting && (
              <div className="flex flex-col md:flex-row space-y-2 md:space-y-0 items-center justify-around my-6">
                <div
                  className="flex flex-col items-center bg-slate-950 p-6 w-full mx-4 rounded-lg cursor-pointer"
                  onClick={() => openFilePicker()}
                >
                  {UploadIcon}
                  <span className="mt-3">Carregar arquivo</span>
                </div>
                <div className="flex flex-col items-center bg-slate-950 p-6 w-full mx-4 rounded-lg cursor-pointer">
                  {MicIcon}
                  <span className="mt-3">Capturar áudio</span>
                </div>
              </div>
            )}
            {isSubmitting && (
